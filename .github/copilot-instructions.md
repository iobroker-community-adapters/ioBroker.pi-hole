...
This adapter integrates **Pi-hole** — a network-wide DNS-based ad blocker — into ioBroker. It allows users to:
- Monitor Pi-hole statistics (blocked queries, top clients, forward destinations, etc.)
- Enable or disable Pi-hole ad-blocking programmatically from ioBroker
- Timed deactivation: disable Pi-hole for a configurable number of seconds, then re-enable automatically
- Retrieve version and update information from the Pi-hole installation

The adapter communicates with the Pi-hole Admin API (`/admin/api.php`) over HTTP or HTTPS, using an API token for authenticated operations (enable/disable). Both HTTP and HTTPS connections are supported, with optional certificate rejection for self-signed certs.
...
#### Pi-hole API Testing Patterns

- Pi-hole does not provide public demo credentials; tests against a live instance require a real Pi-hole token
- Use the `request` library (or native `fetch`) to call the Pi-hole API locally
- Store the Pi-hole API token securely (e.g., in GitHub Secrets as `PIHOLE_TOKEN`)
- Test against a local Pi-hole instance if available in CI, or skip API tests when the token is not set

**Example Implementation:**
```javascript
it("Should connect to Pi-hole API", async () => {
    const piholeIP = process.env.PIHOLE_IP || '192.168.1.1';
    const piholeToken = process.env.PIHOLE_TOKEN;
    
    if (!piholeToken) {
        console.log("⚠️  Skipping Pi-hole API test: PIHOLE_TOKEN not set");
        return;
    }

    await harness.changeAdapterConfig("pi-hole", {
        native: {
            piholeIP: piholeIP,
            piholeToken: piholeToken,
            piholeHttps: false,
            piholeAllCerts: false,
            piholeRenew: 30,
        }
    });
    ...
    const connectionState = await harness.states.getStateAsync("pi-hole.0.info.connection");
    
    if (connectionState?.val === true) {
        console.log("✅ SUCCESS: Pi-hole connection established");
        return true;
    } else {
        throw new Error("Pi-hole connection failed. Check PIHOLE_IP and PIHOLE_TOKEN.");
    }
}).timeout(60000);
```
...
**Pi-hole HTTP Patterns:**
- The adapter currently uses the `request` library for HTTP calls (legacy); prefer migrating to native `fetch` for new features
- Pi-hole supports both HTTP and HTTPS; use adapter config (`piholeHttps`) to select
- For HTTPS with self-signed certificates, use `piholeAllCerts` config to toggle `rejectUnauthorized`
- API endpoints follow the pattern: `http(s)://<piholeIP>/admin/api.php?<endpoint>&auth=<token>`
- Common Pi-hole API endpoints used by this adapter:
  - `summary` / `summaryRaw` — Ad-blocking statistics
  - `topItems` — Top blocked and permitted domains
  - `getQuerySources` — Top query sources
  - `overTimeData10mins` — Traffic data over time
  - `getForwardDestinations` — DNS forward destinations
  - `version` / `versions` — Pi-hole version info
  - `enable` / `disable[=<seconds>]` — Control Pi-hole blocking (requires auth token)
...
**Pi-hole Specific Error Scenarios:**
- Pi-hole unreachable (wrong IP, firewall): log error and set `info.connection` to `false`
- Invalid or expired API token: the Pi-hole API returns `[]` (empty array) for authenticated endpoints — detect and warn the user
- HTTPS certificate errors: configurable via `piholeAllCerts` setting
- Pi-hole API returns unexpected data structures: guard with `typeof` checks before processing
...
## Pi-hole Adapter-Specific Patterns

### API Endpoint Handling

The Pi-hole adapter polls multiple API endpoints on a configurable interval. When adding new endpoints:

1. Add the endpoint name to the `valuePaths` array in `main.js`
2. Ensure the `getPiholeValues()` function handles the new data structure correctly
3. For tabular data (like `topItems`, `getQuerySources`), use the `data-table` state pattern with JSON-serialized content
4. For flat key-value responses (like `summary`, `version`), states are created dynamically from the API response keys

### State Naming Conventions

- States mirror Pi-hole API response keys directly (e.g., `summary.ads_blocked_today`, `version.core_update`)
- Channel names match API endpoint names (e.g., `summary`, `topItems`, `getForwardDestinations`)
- Control states use descriptive names: `deactPiHoleTime` (seconds to disable), `actPiHole` (re-enable button)

### Configuration Properties

Key adapter configuration properties (defined in `io-package.json` and the admin UI):
- `piholeIP` — IP address or hostname of the Pi-hole instance
- `piholeToken` — Pi-hole API authentication token
- `piholeHttps` — Use HTTPS instead of HTTP
- `piholeAllCerts` — Accept all SSL certificates (disable `rejectUnauthorized`)
- `piholeRenew` — Poll interval in seconds (minimum 2)

### Timed Deactivation Pattern

Pi-hole supports timed deactivation via `?disable=<seconds>`. The adapter implements this as:
- Write a number (seconds) to the `deactPiHoleTime` state to disable Pi-hole for that duration
- After writing, a 1-second timeout refreshes the `summary` and `summaryRaw` states
- The `actPiHole` boolean state re-enables Pi-hole immediately
